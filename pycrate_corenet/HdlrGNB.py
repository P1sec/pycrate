...
class GNBd(object):
    ...
    def _log(self, logtype, msg):
        ...
        if logtype[:3] == 'TRA':
            ...
            log('[%s] [GNB: %s.%s] %s' % (logtype, self.ID[0], self.ID[1], msg))
    ...
    def get_ngsetup_ies_from_cfg(self):
        ...
        """
        ies = {'AMFName'            : self.Server.ConfigN2['AMFName'],
               'PLMNSupportList'    : cplist(self.Server.ConfigN2['PLMNSupportList']),
               'RelativeAMFCapacity': self.Server.ConfigN2['RelativeAMFCapacity'],
               'ServedGUAMIList'    : cplist(self.Server.ConfigN2['ServedGUAMIList']),
               }
        if 'UERetentionInformation' in self.Server.ConfigN2:
            ies['UERetentionInformation'] = self.Server.ConfigN2['UERetentionInformation']
        return ies
